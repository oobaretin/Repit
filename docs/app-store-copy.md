# Repit — App Store Copy

Use this when creating the listing in App Store Connect.  
**Bundle ID:** `com.repit.app` · **Category:** Health & Fitness · **Version:** 1.0.0

---

## App Name

**Repit**

(30 characters max — current: 5)

---

## Subtitle

**Mindfulness Repetition Timer**

(30 characters max — current: 28)

---

## Promotional Text

Optional. Can be updated without a new app review.

> Count mantras, affirmations, or breaths with a calm repetition timer. Focus lock, gentle sounds, haptics, and on-device Face ID — built for daily practice.

(169 characters)

---

## Description

```
Repit is a minimal repetition timer for mantra practice, affirmations, breath counting, and any mindful practice that uses a steady count.

Set your target repetitions and interval, tap Start, and follow the breathing circle. Each rep brings a gentle sound and haptic tap so you can stay present without watching the screen.

FEATURES

• Repetition timer — set any target count and interval from 0.5 to 10 seconds
• Quick presets — 27, 54, 108, and 1000 for traditional practice lengths
• Breathing circle — visual rhythm synced to your interval
• Sounds — Mala bead, Gong, Crystal, or silent
• Haptic feedback — light tap on each repetition (iPhone)
• Focus lock — hide controls during a session; hold to unlock when needed
• Session tracking — lifetime sessions and total reps stored on your device
• Privacy first — no account required; Face ID unlock stays on your device

WHO IT'S FOR

• Mantra and japa meditation
• Affirmation practice
• Breath counting
• Mala bead counting without physical beads
• Any repetition-based mindfulness routine

Repit stays out of your way: dark, elegant, and designed for uninterrupted practice.

Questions or feedback? Open an issue on GitHub or contact us through the support link in the listing.
```

(~1,350 characters — well under the 4,000 limit)

---

## Keywords

Apple allows up to **100 characters**. No spaces after commas. Do not repeat words from the app name or subtitle.

```
mantra,meditation,breath,mala,affirmation,mindfulness,japa,108,count,repetition,focus,calm,timer
```

(96 characters)

**Alternate set** (swap if you A/B test later):

```
mantra,meditation,timer,breath counting,mala beads,affirmation,japa,108,repetition,focus,calm
```

---

## What's New (Version 1.0)

```
Welcome to Repit — a mindfulness repetition timer for mantra, affirmations, and breath counting.

• Configurable reps and interval
• Breathing circle synced to your pace
• Mala, Gong, and Crystal sounds
• Focus lock and Face ID app lock
• Session stats stored on device
```

---

## URLs

| Field | Value |
|-------|--------|
| **Privacy Policy URL** | `https://repit-privacy.netlify.app/` |
| **Support URL** | `https://github.com/oobaretin/Repit/issues` |
| **Marketing URL** | *(optional)* `https://github.com/oobaretin/Repit` |

Update the privacy URL if you use a custom domain later.

---

## App Store Connect — Privacy Questionnaire (summary)

Based on current app behavior:

| Data type | Collected? | Linked to user? | Used for tracking? |
|-----------|------------|-----------------|-------------------|
| Contact info | No | — | — |
| Health & fitness | No* | — | — |
| Financial info | No | — | — |
| Location | No | — | — |
| User content | No | — | — |
| Identifiers | No | — | — |
| Usage data / analytics | No | — | — |
| Diagnostics | No | — | — |

\* Session rep counts are stored **locally on device only** — not transmitted. Apple may still ask about “Fitness” if you describe breath counting; answer based on whether you claim health features (Repit is a general mindfulness timer, not a medical/health tracker).

**Face ID:** Used for app unlock only; processed on-device via iOS — declare if prompted under authentication/biometric usage.

---

## Age Rating

Expect **4+** — no restricted content, no social features, no user-generated content shared online.

---

## Copyright

`© 2026 [Your Name or Legal Entity]`

Replace with your legal name before submission.

---

## Review Notes (for Apple)

Optional field when submitting:

```
Repit is a repetition timer for mindfulness practice. No account is required.

To test Face ID lock: open Settings → tap Logout → unlock with Face ID.
To test focus lock: start a session → controls hide automatically → hold the unlock button at the bottom for ~1.2 seconds.

All session data is stored locally on the device.
```
